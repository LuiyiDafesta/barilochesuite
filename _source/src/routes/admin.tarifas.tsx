...
function Tarifas() {
  const [base, setBase] = useState(165000);
  const [weekend, setWeekend] = useState([15]);
  const [cleaning, setCleaning] = useState(38000);

  return (
    <div className="space-y-6">
      <PageHeader
        title="Tarifas"
        description="Motor de precios: tarifa base, recargos y reglas por temporada."
        actions={
          <Button size="sm" className="rounded-full" onClick={() => toast.success("Tarifas actualizadas")}>
            Guardar cambios
          </Button>
        }
      />
